...
def train_single_scale(netD_a, netD_b, netG_a, netG_b, data_loader_a, reals_b, fixed_noise_a,fixed_noise_b, noise_amp_a,noise_amp_b, fakes_a,fakes_b, mixs_g_a, mixs_g_b, opt, depth):
    ...
    for iter in _iter:
        ...
        for reals_a in data_loader_a:
            ...
            #print([other_noise_b[i].shape for i in range(len(other_noise_b))])
            #print([real_b[i].shape for i in range(len(real_b))])
            realss_b =[]
            realss_a =[]
            ...
            noisy_real_b = [other_noise_a[i]*noise_amp_a[i] + realss_b[i] for i in range(len(other_noise_a))]
            noisy_real_a = [other_noise_b[i]*noise_amp_b[i] + realss_a[i] for i in range(len(other_noise_b))]
            ...
            if opt.lambda_tv > 0.0:
                loss_tv = TVLoss()
                if (opt.lambda_self > 0.0):
                    loss_tv_a = loss_tv(self_a)
                    loss_tv_b = loss_tv(self_b)
                else:
                    loss_tv_a = 0 
                    loss_tv_b = 0
                tv_loss_a = opt.lambda_tv*(loss_tv(fake_a) + loss_tv(mix_g_a) + loss_tv_a)
                tv_loss_b = opt.lambda_tv*(loss_tv(fake_b) + loss_tv(mix_g_b) + loss_tv_b)
            else:
                output_tv_a = 0
                output_tv_b = 0
            ...
            if beta != 0:
                ...
                if (depth != 0):
                    cycle_a = netG_a(mixs_g_b,reals_shapes, noise_amp_b, is_noise =True) #if else chỗ này tai scale thứ 2 trở đi tăng chanel lên 64 
                    cycle_b = netG_b(mixs_g_a,reals_shapes, noise_amp_a, is_noise = True)
                else:
                    cycle_a = netG_a(mixs_g_b,reals_shapes, noise_amp_b) #if else chỗ này tai scale thứ 2 trở đi tăng chanel lên 64 
                    cycle_b = netG_b(mixs_g_a,reals_shapes, noise_amp_a)        
        ...
        functions.save_image('{}/cycle_a{}.jpg'.format(opt.outf,iter+1),cycle_a.detach())
        functions.save_image('{}/cycle_b{}.jpg'.format(opt.outf,iter+1),cycle_b.detach())
    print(len(errD_a_plot), len(errG_a_plot))       
